...
namespace BubbleGauntlet {
    static class UIHelpers {
        ...
        private static string MakeTitleCharacter(this char ch, int size = 130) {
            string voffset = "0.1";
            if (ch == 'F' || ch == 'f')
                voffset = "0.2";

            return $"<voffset={voffset}em><font=\"Saber_Dist32\"><color=#672B31><size={size}%>{ch}</size></color></font></voffset>";
        }

        public static string MakeTitle(this string str, int size = 130) {
            if (str.Length == 0)
                return "";

            var ret = str[0].MakeTitleCharacter((int)(size * 1.3f));
            if (str.Length > 1)
                ret += str.Substring(1);
            return ret;
    }
