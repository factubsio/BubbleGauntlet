...
namespace BubbleGauntlet.Utilities {
    ...
    public class RollTable<T> where T : IWeighted {

        private readonly List<T> Entries = new();
        private readonly List<(int begin, int end)> Weights = new();
        private int Total = 0;

        public void Add(T item) {
            int weight = item.Weight;
            Weights.Add((Total, Total + weight));
            Entries.Add(item);
            Total += weight;
        }

        public T Next => Get().Item;

        public (int RollResult, T Item) Get() {
            int roll = UnityEngine.Random.Range(0, Total);
            int index = Weights.BinarySearch((roll, roll), EntryComparer.Instance);
            return (roll, Entries[index]);
        }
        ...
        public string Render {
            get {
                ...
                for (int i = 0; i < Weights.Count; i++) {
                    s.AppendLine($"[{Weights[i].begin}-{Weights[i].end - 1}]    {Entries[i]}");
    ...
    public static class Helpers {
        ...
        public static bool Contains(this UnityEngine.RangeInt range, int val) {
